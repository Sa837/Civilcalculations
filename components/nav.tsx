      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
        ...
        <Link href="/" className="flex items-center gap-3 transition-opacity hover:opacity-80">
          ...
          <div className="flex h-12 w-12 items-center justify-center  ` rounded-full shadow-lg bg-gradient-to-tr from-blue-600 via-indigo-500 to-cyan-400 text-white">
            {/* The SVG is now clean, minimal, and relies on the parent's gradient for color */}
            <svg
              viewBox="0 0 150 100"
              className="h-9 w-9" /* Smaller size for better fit inside the 12x12 container */
              xmlns="http://www.w3.org/2000/svg"
              fill="none" /* Set fill to none initially for clear separation of concerns */
            >
              {/* Rectangular background element is REMOVED. The parent <div> now provides the gradient background. */}

              {/* The CC text: Use current color from the Tailwind parent div (text-white) */}
              <text
                x="10"
                y="80"
                fontSize="100" /* Use camelCase for React/JSX SVG attributes */
                fontWeight="bold"
                fill="currentColor" /* Inherits color (white) from the parent div */
                letterSpacing="-5" /* Use camelCase for React/JSX SVG attributes */
              >
                CC
              </text>
            </svg>
        ...
        <nav className="hidden items-center gap-10 md:flex">
          ...
          <div className="group relative">
            ...
            <div className="invisible absolute left-1/2 z-40 mt-5 w-[400px] -translate-x-1/2 rounded-2xl border border-slate-200/20 bg-surface/95 p-6 opacity-0 shadow-hover backdrop-blur-xl transition-all group-hover:visible group-hover:opacity-100 dark:border-slate-800/20 dark:bg-surface-dark/95">
              <p className="mb-3   text-xs font-semibold uppercase tracking-wider text-body/60 dark:text-body-dark/60">
                Popular Converters
              </p>
          ...
          <div className="mx-auto max-w-7xl px-6 pb-8">
            ...
            <div className="grid gap-2">
              <Link
                href="/"
                onClick={() => setOpen(false)}
                className={`rounded-xl px-5 py-4   font-medium transition-colors ${
                  isActive('/')
