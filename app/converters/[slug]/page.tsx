...
export default function ConverterDetail({ params }: Params) {
  ...
  if (!converter)
    return (
      <main className="mx-auto max-w-4xl px-6 py-16">
        <p>
          Converter not found.{' '}
          <Link className="text-accent underline" href="/converters">
            Back
          </Link>
        </p>
      </main>
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      <Link
        href="/converters"
        className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
      >
        ← Back to Converters
      </Link>
      <div className="mb-2">
        <div className="flex items-center gap-3">
          ...
          <span className="rounded-full bg-primary/10 px-3 py-1 text-[10px] font-sans font-semibold uppercase tracking-wide text-primary dark:bg-primary/20">
            {converter.category}
          </span>
        </div>
        {converter.description && (
          <p className="mt-1 font-sans text-slate-600 dark:text-slate-300">
            {converter.description}
          </p>
        )}
      </div>
      ...
      {isDateConverter ? (
        <div className="mt-8">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <form
              className="grid gap-4 rounded-2xl border border-slate-200 p-4 dark:border-slate-800"
              onSubmit={handleDateConvert}
            >
                ...
                {convertClicked && dateResult ? (
                  <div className="flex flex-col gap-1">
                    <span className="text-lg font-semibold text-slate-500">
                      {dateTab === 0 ? 'Nepali (BS) Date:' : 'English (AD) Date:'}
                    </span>
                    <span className="text-2xl font-bold text-primary">{dateResult}</span>
                  </div>
                ) : (
                  <span className="text-slate-500 font-sans">
                    Enter a date and click Convert to see the result.
                  </span>
                )}
              </div>
            </div>
          ...
          >
            ...
            <div className="grid gap-1">
              ...
              <select
                ...
                className="rounded-xl border border-slate-300 bg-white px-3 py-2 font-sans font-medium dark:border-slate-700 dark:bg-slate-900"
              >
                {units.map((u: any) => (
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <input
                ...
                className="w-20 rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-900"
              />
            </div>
          </form>
          <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
              ...
              {from === '' ? (
                ...
                <div className="flex items-center gap-2">
                  <p className="text-2xl font-bold">
                    {result.toLocaleString(undefined, { maximumFractionDigits: precision })}
                  </p>
                  <button
                    type="button"
                    onClick={async () => {
            ...
            {converter.baseUnit && from && !isNaN(Number(from)) && (
              <div className="mt-4 text-sm text-slate-600 dark:text-slate-400">
                <p className="font-medium">Conversion details:</p>
                <p className="mt-1">Base unit: {converter.baseUnit}</p>
